...
function _compressImage(file: File): Promise<Blob | null> {
  return new Promise((resolve) => {
    try {
      ...
      img.onload = () => {
        ...
        canvas.toBlob((blob) => { URL.revokeObjectURL(objUrl); resolve(blob); }, "image/jpeg", 0.75);
      };
...
async function uploadUtenteDoc(utenteId: string, file: File): Promise<string | null> {
  try {
    if ((file.type || "").startsWith("image/")) {
      const _c = await _compressImage(file);
